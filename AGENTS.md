...
- `app/(admin)/shifts/` の月次カレンダービュー実装（react-big-calendar）
- dnd-kit によるドラッグ＆ドロップでのシフト移動
- 編集時のリアルタイム制約チェック（`lib/constraints/` を呼び出す）
- 制約違反時の警告バッジ表示
- 一覧リスト表示モード（月次カレンダーとの切替）
- `app/(staff)/my-shifts/` のスタッフ用シフト確認画面（`published` 以降のみ表示）
...
- 制約違反時に警告が表示される
- スタッフ画面で自分の `published` シフトのみ閲覧できる
...
- `app/api/shifts/export/route.ts` で ExcelJS を使ったサーバーサイド生成を実装
- 2シート構成（月次シフト表 + スタッフ一覧）を作成する
- セル色・罫線・土日祝ハイライト・集計 SUM 数式を適用する
...
- 集計値を Python や JavaScript で計算してセルに直接埋め込むこと（Excel SUM 数式を使うこと）
