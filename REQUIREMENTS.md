...
- 管理者とスタッフは同一ログインページから施設ID + メール + パスワードでログイン
...
| **Pro** | 月3,000〜5,000円 / 施設 | 中規模施設・本格運用 |
| **Enterprise** | 要相談 | 大規模・複数施設展開 |
...
### 9.3 DB 実装方針
```sql
-- facilities テーブルに plan カラムを追加
alter table facilities
  add column plan text not null default 'free'
    check (plan in ('free', 'pro', 'enterprise')),
  add column plan_expires_at timestamptz;  -- null = 無期限 or 無料プラン
```
